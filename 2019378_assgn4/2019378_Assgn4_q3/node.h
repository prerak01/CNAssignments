 ...
 public:
  ...
  void sendMsg(){
    ...
    for(int j=0;j<ntbl.tbl.size();j++){
      string to= interfaces[i].first.getConnectedIp();
      if(ntbl.tbl[j].nexthop==to)
        ntbl.tbl[j].cost=16;
    }
      ...
      // b and c unreachable
      if(!(tableUpdate && (msg.from == "10.0.1.23" || msg.from=="10.0.1.3")))	
        interfaces[i].second->recvMsg(&msg);

    }
  }

  // bool unreachable(string destination){

  //   for(int i=0;i<mytbl->tbl.size();i++){
  //     if(mytbl->tbl[i].dstip== destination && mytbl->tbl[i].cost>15)
  //       return true;
  //   }
  //   return false;
  // }
